...
class GetChinaNews():
	...
	def getNewsFromRollPage(self,site):
		...
		newsUrl = "NONE"
		#设置m[0:5]的值来决定输出的新闻的条数，若不设置默认输出所有的新闻内容
		for i in m[0:3]:
			try:
				#从网页上获取新闻属性
				...
				print "Something Wrong when getting "+site+" news:"+newsUrl
		return True
	...
	def getNewsFromSite(self,site):	
		...
		print "Please check news in file:", argD+ "/"+self.strYear+self.strMonth+self.strDay+"/"
		
	def getChinaNews(self):
		try:
			for tt in self.date_range:
				#print "Get News From Date:"+str(tt)
				print "Getting News, please wait."
			...
			with open(newsListFilePath+'newslist.txt','wa') as newslistFile:
				...
				for i in self.newsList:
					uni = i[0].decode('gb2312')
					test11 = uni.encode('utf8')
					print test11
			...
			print "Something wrong when getting chinanews."
